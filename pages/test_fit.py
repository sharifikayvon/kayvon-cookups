...
# functions a user is allowed to reference in a model formula, plus x itself
ALLOWED_FUNCS = {
    "sqrt": np.sqrt,
    "exp": np.exp,
    "ln": np.log,
...
if "xdata" in locals() and "ydata" in locals():
    ...
    if has_valid_xy(xdata, ydata):
        ...
        if any_fit_plotted:
            ax.legend(fontsize=10, labelspacing=1.2)
...
# def fmt_fitted_formula(formula, params, popt):
#     """Substitute fitted parameter values into the formula string for display,
#     longest names first so e.g. 'a1' doesn't get clobbered by a match on 'a'."""
#     out = formula
#     for name, val in sorted(zip(params, popt), key=lambda p: -len(p[0])):
#         out = re.sub(rf"\b{re.escape(name)}\b", fmt_value(val), out)
#     return out
...
#                 fitted_str = fmt_fitted_formula(formula, params, popt)
#                 param_str = ", ".join(
#                     f"{name} = {fmt_value(val)}" for name, val in zip(params, popt)
#                 )

#                 color = fit_colors[i % len(fit_colors)]
#                 ax.plot(x_fit, y_fit, color=color, lw=3, label=f"y = {fitted_str}")
#                 any_fit_plotted = True

#                 st.caption(f"model {i + 1}: y = {formula}  →  {param_str}")
